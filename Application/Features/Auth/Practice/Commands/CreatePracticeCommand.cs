...
{
    public class CreatePracticeCommand : IRequest<Response<CreateRequestResponse>>
    {
    ...
    public class CreatePracticeCommandHandler : IRequestHandler<CreatePracticeCommand, Response<CreateRequestResponse>>
    {
        private readonly IPracticeRepositoryAsync _practiceRepository;
        private readonly IMapper _mapper;
        public CreatePracticeCommandHandler(IPracticeRepositoryAsync practiceRepository, IMapper mapper)
        {
            _practiceRepository = practiceRepository;
            _mapper = mapper;
        }

        public async Task<Response<CreateRequestResponse>> Handle(CreatePracticeCommand request, CancellationToken cancellationToken)
        {
            var practice = _mapper.Map<Domain.Entities.Auth.Practice>(request);
            var userStatus = await _practiceRepository.AddPracticeAsync(practice).ConfigureAwait(false);
